...
- [Top to Bottom Text](#top-to-bottom-text)
...
`TOGL_SetOrigin(TOGL_ORIGIN_ID_LEFT_TOP)` - Sets coordinates system origin to left-top for drawing text.
Text will be rendered from top to bottom.
...
int RunTopToBottomText() {
    ...
    data.window_name        = "Top to Bottom Text";
    data.area               = {0, 0, s_size.width, s_size.height};
    // Ignores data.area.x and data.area.y coordinates 
    // and centers window in middle of work area (screen area excluding task bar).
    data.style              |= TOGL_WINDOW_STYLE_BIT_CENTERED;
    ...
    data.do_on_create = []() {
        ...
        TOGL_SetOrigin(TOGL_ORIGIN_ID_LEFT_TOP);
    ...
    data.draw = []() {
        ...
        TOGL_RenderText({0, 0}, {255, 255, 255, 255}, "This is simple text.\nX - Exit.");
    };
