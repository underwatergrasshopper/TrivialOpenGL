...
namespace TrivialOpenGL {
    ...
    inline bool IsEnabled(WindowOption window_option) {
        return ToWindow().IsEnabled(window_option);
    }
    ...
    // Resizes window and keeps current window position.
    inline void Resize(uint16_t width, uint16_t height, bool is_draw_area = false) {
        ToWindow().Resize(width, height, is_draw_area);
    }
    inline void Resize(const SizeU16& size, bool is_draw_area = false) {
